...
class MakeListExercises extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:3001/api/exos")
      .then(res => res.json())
      .then(
        result => {
          this.setState({
  ...
  renderListExercises() {
    ...
    for (let i = 0; i < this.state.items.length; i++) {
      ...
      if (this.state.items[i].doable === false) {
  ...
  renderInfoExercises = numExercise => {
    ...
    if (Object.keys(this.state.items[numExercise]).length === 7) {
      let TitleToRender = "";
      ...
      if (this.state.items[numExercise].premisses[0]) {
        PremissesToRender = (
          <Fragment>
            {this.renderLiInformationExercice(
              this.state.items[numExercise].premisses
            )}
          </Fragment>
        );
      }
      ...
      if (this.state.items[numExercise].rulesImplied[0]) {
        RulesImpliedToRender = (
          <Fragment>
            {this.renderLiInformationExercice(
              this.state.items[numExercise].rulesImplied
            )}
          </Fragment>
        );
      }
      ...
      this.setState({
        arrayInfoExercise: (
          <Fragment>
            ...
            Conclusion : <li>{this.state.items[numExercise].conclusion}</li>
